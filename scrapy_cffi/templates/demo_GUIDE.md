...
| `scrapy-cffi demo -tls` | Standalone TLS inspection and explicit impersonation |
...
The framework release verifier runs interrupt verification only for the three
queue-backed continuous modes. Memory Demo is finite; artificially holding it
open would not be valid continuous-listener evidence.
...
The generated queue demos receive one seeded input and set
`start_request_limit = 1`, so their producer completion is a real ingress
event. Their verifier waits for `runner.py` to finish naturally. A production
consumer normally keeps the default `start_request_limit = None` and remains
subscribed until an explicit shutdown event. Empty broker reads and elapsed
delays never signal completion. A finite Engine waits for its own HTTP
callbacks and WebSocket listeners, so another spider sharing the same Crawler
cannot keep it alive or make it exit early.
