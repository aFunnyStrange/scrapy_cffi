...
Write-Warning "Deleting all data volumes owned by Compose project '$ProjectName'."
& docker compose --project-name $ProjectName --env-file $envFile --file $composeFile down --volumes --remove-orphans
if ($LASTEXITCODE -ne 0) {
    throw "Development infrastructure reset failed while removing the old stack."
}

& (Join-Path $infraDir "init.ps1") -Topology $Topology -ProjectName $ProjectName
